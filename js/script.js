...
document.addEventListener("DOMContentLoaded", () => {
  ...
  let chartData = {
    ...
    datasets: [
      {
        label: "Shoes",
        data: [300, 450, 500, 350, 550, 400, 330], // Jumlah penjualan awal
        backgroundColor: [
          "rgba(255, 99, 132, 0.2)",
          "rgba(54, 162, 235, 0.2)",
          "rgba(255, 206, 86, 0.2)",
  ...
  function updateChartFromStorage() {
    ...
    Object.keys(storageData).forEach((productName) => {
      const productIndex = chartData.labels.indexOf(productName);

      if (productIndex !== -1) {
        // Tambahkan jumlah ke produk yang sudah ada
        chartData.datasets[0].data[productIndex] += storageData[productName];
      } else {
        // Tambahkan produk baru
        chartData.labels.push(productName);
        chartData.datasets[0].data.push(storageData[productName]);
      }
    });
  ...
  window.onload = function () {
    handleAddToCartButtons();
    updateCartBadge();
    updateChartFromStorage(); // Update chart dengan data dari localStorage
  };
  ...
  document.getElementById("checkoutButton").addEventListener("click", () => {
    // Periksa apakah ada produk di cart
    if (listCart.length > 0) {
      listCart.forEach((product) => {
        updateChart(product.name, product.quantity); // Tambahkan ke chart
      });
      ...
      showToast("Checkout successful! Chart updated.", "success");
    } else {
